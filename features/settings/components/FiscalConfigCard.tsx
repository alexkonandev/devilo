...
const TAX_MASKS: Record<string, string> = {
...
// ═══════════════════════════════════════════════════════════════════════════════
// FiscalConfigCard — Configuration fiscale & numérotation des devis
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    <div className={cn(STUDIO_V2_CARD, className)}>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="relative">
              <button
                ...
                className={cn(DS_INPUT, DS_ROUNDED, "font-sans w-full text-xs text-left flex items-center justify-between")}
              >
              ...
              {showCurrencyDropdown && (
                <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-white border border-slate-100 rounded-md max-h-40 overflow-y-auto shadow-sm">
                  <input
                    ...
                    className="w-full px-2 py-1.5 text-[10px] border-b border-slate-100 focus:outline-none"
                  />
                  {filteredCurrencies.map((currency) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setCurrencySearch("");
                        if (!watchedValues.taxIdLabel || watchedValues.taxIdLabel === "NCC") {
                          const taxConfig = TAX_LABELS[currency.code as keyof typeof TAX_LABELS];
                          const suggestedLabel = typeof taxConfig === "string" ? taxConfig : (taxConfig as Record<string, string>)?.FR || "NCC";
                    ...
                    >
                      <span className="font-sans font-medium">{currency.code}</span>
          ...
          <div>
            ...
            <input
              ...
              type="number"
              step="0.01"
              className={cn(DS_INPUT, DS_ROUNDED, "font-sans w-full text-xs text-right")}
              placeholder="18.00"
            />
            {errors.defaultVatRate && (
              <p className="mt-0.5 text-[9px] text-rose-500">{errors.defaultVatRate.message}</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-md border border-slate-200 bg-slate-50/40 px-2.5 py-2 space-y-2">
          <div className="flex items-center gap-1.5">
            <ReceiptIcon size={8} className="text-slate-500" />
            <span className="text-[7.5px] font-sans font-semibold uppercase tracking-wider text-slate-600">
              Identification Fiscale
            </span>
          </div>
            ...
            <div>
              <div className="relative">
                <button
                  ...
                  className={cn(DS_INPUT, DS_ROUNDED, "font-sans w-full text-xs text-left flex items-center justify-between uppercase bg-white")}
                >
                    ...
                    {TAX_LABEL_OPTIONS.map((option) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (TAX_MASKS[option.value]) setTaxMask(TAX_MASKS[option.value]);
                        }}
                        className="w-full px-2 py-1.5 text-[10px] text-left hover:bg-slate-50 flex items-center justify-between"
                      >
                        <span className="font-sans font-medium">{option.value}</span>
              ...
              <span className="block text-[7px] text-slate-500 mt-0.5">Label fiscal</span>
            </div>
            <div>
              <div className="relative">
                <input
                  {...register("taxId")}
                  className={cn(DS_INPUT, DS_ROUNDED, "font-sans w-full text-xs bg-white")}
                  placeholder={taxMask || "CI-ABJ-2023-A-12345"}
                  onInput={(e) => {
                    const target = e.target as HTMLInputElement;
                    const cleaned = target.value.replace(/\s+/g, " ").replace(/-+/g, "-").replace(/\/+/g, "/").trim();
                    setValue("taxId", cleaned);
                  }}
                />
                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[8px] text-slate-500">
                  {watchedValues.taxIdLabel || "NCC"}
                </span>
              </div>
              <span className="block text-[7px] text-slate-500 mt-0.5">Numéro</span>
              {errors.taxId && (
                <p className="text-[7px] text-rose-500 mt-0.5">{errors.taxId.message}</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <input
                ...
                className={cn(DS_INPUT, DS_ROUNDED, "font-sans w-full text-xs uppercase bg-white")}
                placeholder="QT-"
              />
              <span className="block text-[7px] text-slate-500 mt-0.5">Préfixe</span>
              {errors.quotePrefix && (
                <p className="text-[7px] text-rose-500 mt-0.5">{errors.quotePrefix.message}</p>
              )}
            </div>
            <div>
              <input
                ...
                placeholder="1"
              />
              <span className="block text-[7px] text-slate-500 mt-0.5">Prochain N°</span>
              {errors.nextQuoteNumber && (
                <p className="text-[7px] text-rose-500 mt-0.5">{errors.nextQuoteNumber.message}</p>
              )}
            </div>
          </div>
        </div>
